...
EOT
    ...
    echo "🕒 Configuring time synchronization (chrony)..."
    sudo apt-get update -qq >/dev/null
    sudo apt-get install -y -qq chrony >/dev/null
    # Comment out existing pool/server lines
    sudo sed -i -e 's/^pool .*/#&/' -e 's/^server .*/#&/' /etc/chrony/chrony.conf
    # Add ntp.nict.jp
    echo 'server ntp.nict.jp iburst' | sudo tee -a /etc/chrony/chrony.conf
    sudo systemctl restart chrony
    echo "--- Waiting 10 seconds for chrony to sync..."
    sleep 10
    chronyc tracking
